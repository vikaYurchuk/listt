...
export async function addItem(title: string): Promise<Task> {
  const result = await db.runAsync(`INSERT INTO products (title) VALUES (?);`, [
    title,
  ]);
  return { id: result.lastInsertRowId, title } as Task;
}
...
export async function updateItem(item: Task) {
...
export async function getItems() {
  return await db.getAllAsync<Task>("SELECT * FROM products");
}